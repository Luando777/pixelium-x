console.log("Pixelium System Online V9 BASE64");
alert("✅ SISTEMA V9 (WEB): Reparación Directa Disponible (Sin Storage).");
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.uploadRepairImage = (index, prodId) => {
        ...
        // V9 Logic: Canvas Resize + Base64 -> Firestore Direct
        const reader = new FileReader();
        reader.onload = function (e) {
            ...
            img.onload = function () {
                ...
                // Resize logic (Max 600px - Aggressive resize for Firestore limit)
                const MAX_WIDTH = 600;
                ...
                // Export as JPEG 0.7 quality to ensure small size (<500KB)
                const dataUrl = canvas.toDataURL('image/jpeg', 0.7);

                // Hard check size (Max Firestore doc is 1MB)
                // dataUrl length ~= bytes * 1.33. So 800k chars is safe.
                if (dataUrl.length > 800000) {
                    alert("❌ La imagen es demasiado compleja. Intenta con una más simple.");
                    btn.innerText = "❌ Muy pesada";
                    setTimeout(() => { btn.innerText = originalText; btn.disabled = false; }, 2000);
                    return;
                }

                btn.innerText = "⏳ Guardando (V9)...";

                db.collection('products').doc(prodId).update({ image: dataUrl })
                    .then(() => {
                        alert("✅ ¡ÉXITO V9 (Web)! Imagen reparada.");
                        fileInput.value = '';
                        btn.innerText = "✅ Listo";
                        setTimeout(() => { btn.innerText = originalText; btn.disabled = false; }, 2000);
                    })
